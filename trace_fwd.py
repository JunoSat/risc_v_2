...
with open(vcd_path, 'r') as f:
    ...
    for line in f:
        ...
        if time == 775000 and line.startswith('#'):
            print(f"Time {time}:")
            print(f"ex_mem_rd={state['ex_mem_rd']} rs2={state['id_ex_rs2']}")
            print(f"ex_mem_reg_write={state['ex_mem_reg_write']}")
            print(f"forward_b={state['forward_b']}")
