...
with open(vcd_path, 'r') as f:
    ...
    for line in f:
        ...
        if time == 765000 and line.startswith('#'):
            print(f"Time {time}:")
            print(f"if_id_inst={hex(int(state['if_id_inst'],2)) if state['if_id_inst'] else '0'}")
            print(f"id_ex_inst={hex(int(state['id_ex_inst'],2)) if state['id_ex_inst'] else '0'}")
